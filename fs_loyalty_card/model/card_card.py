...
class CardCard(models.Model):
    ...
    last_period_total_point = fields.Float(
        string='Total des points/dernière période',
    ...
    card_expected_date = fields.Date(
        string='Date prévue (Réception de la Hard Card)')
    ...
    def convert_amount_to_point(self, amount):
        if not amount: #or prate < 0:
            return 0.00
        res = float(amount)/self.type_id.categ_id.Montant_requis#res = float(amount)/float(prate) (change)
        return int(res)
